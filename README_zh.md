...
- 在脚本或自定义节点中，可调用 `src.game.config.resolve_behavior_mode(state=state)`，统一获取当前模式并兼容覆盖/默认值。
- 通过命令行体验策略日志输出：
  ```bash
...
│   │   ├── modes/            # 各模式的行为、节点与工具箱
│   │   │   ├── shared/       # 行为接口与注册表
│   │   │   ├── workflow/     # Workflow 模式的行为与节点委托
│   │   │   └── agent/        # Agent 模式的行为、记忆与工具
│   │   ├── nodes/            # 根据模式调度 modes/* 的适配层
│   │   └── rules.py          # 游戏逻辑和胜利条件
│   ├── tools/
│   │   └── llm/              # LLM 客户端工具以及推理/发言模块
├── tests/                    # Pytest 测试套件
